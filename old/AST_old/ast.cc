...
Type* getCommonType(Type* _t1, Type* _t2, bool second_pass)
{
	...
	except(E_INTERNAL, "one of the types is null");
    auto t1 = _t1->drill();
    auto t2 = _t2->drill();
    ...
    if(second_pass)
	except(E_NONEQUIVALENT_TYPES, "Failed To Get Common Type Between " + t1->str() + " and " + t2->str());
...
Type* getCommonType(std::vector<Type*> items)
{
	...
	return items[0];
    if(items.size() == 0)
	except(E_INTERNAL, "Cannot get universal common type of empty list");
...
int getCompatabilityScore(MethodSigStr& func, MethodSigStr mock)
{
	...
	if(mock.generics.size()) {
		...
		*/
		...
		if(!refersTo)
		    except(E_UNRESOLVED_TYPE, "Failed To Get Type For Generic Param " + name);
