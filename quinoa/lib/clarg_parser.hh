...
class ClargParser {
  ...
  public:
    ...
    template <typename T> T get_clarg(std::string name) {
        if (std::is_same<T, std::string>())
            return *(std::string*)get_ptr_to_clarg(name, ArgType::STR);
        else if (std::is_same<T, int>())
            return *(std::string*)get_ptr_to_clarg(name, ArgType::INT);
        else if (std::is_same<T, bool>())
            return *(std::string*)get_ptr_to_clarg(name, ArgType::BOOL);
        // TODO: Find a way to hook this into the compiler properly
        else
            except(E_INTERNAL, "Invalid get_clarg template detected");
    }
        ...
        while (args.size()) {
            ...
            if (current[0] == '-') {
                ...
                if (!entry.initialized)
                    except(E_BAD_ARGS, "Flag '-" + flag_name + "' is not a recognized argument");
...
#define f(a)                                                                                                           \
    if (lowered != #a) {                                                                                               \
        ...
        break;                                                                                                         \
                            ...
                            t(yes) t(y) t(true) t(t) f(no) f(n) f(false) f(f)
...
#undef f
                ...
                case ArgType::STR: {
                    if (!args.size())
                        except(E_BAD_ARGS, message_prefix + " expects a string");
                    *(std::string*)entry.value = popf(args);
                    break;
                }
                default:
                    except(E_INTERNAL, "Bad Argument Type");
                }
            } else if (current[0] == '"') {
                Logger::debug("print not supported");
                exit(1);
            } else
                list_args.push_back(current);
        }
    }
  ...
  private:
    void* get_ptr_to_clarg(std::string name, ArgType expected_type) {
        ...
        if (!entry.initialized)
            except(E_INTERNAL, "nonexistant argument: " + name);
        if (entry.type != expected_type)
            except(E_INTERNAL, "Incorrect type for arg: " + name + " - " + std::to_string(entry.type));
        if (!entry.value)
            except(E_INTERNAL, "Uninitialized arg: " + name);
        return entry.value;
    }
