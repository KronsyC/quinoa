...
void mangleVarNames(SourceBlock& code, std::vector<std::string>& occupiedNames){
	for(auto member:code.items){
		if(instanceof<InitializeVar>(member)){
			...
			if(includes(occupiedNames, name->str()))error("Multiple Variables with the same name are currently not supported");
...
void hoistVarInitializations(CompilationUnit& unit){
	...
	for(auto member:unit.items){
		if(instanceof<Module>(member)){
			...
			for(auto member:mod->items){
				if(instanceof<Method>(member)){
					auto method = (Method*)member;
