...
llvm::Function* make_fn(MethodSignature& f,
    llvm::Module* mod,
...
generateModule(Module& mod, std::vector<TopLevelExpression*> injectedDefinitions, CompilationUnit& ast)
{
...
llvm::Module* Codegen::codegen(CompilationUnit& ast)
{
	...
	if(instanceof <Module>(unit)) {
	    ...
	    entrySig.name = new TLCMemberRef(nullptr, Ident::get("main"));
	...
	} else if(instanceof <MethodPredeclaration>(unit) || instanceof <PropertyPredeclaration>(unit)) {
	    defs.push_back(unit);
	} else
