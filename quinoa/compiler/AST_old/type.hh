...
public:
	...
	Type *copy(SourceBlock *ctx)
	{
		auto lt = new ListType(elements->copy(ctx), size);
		lt->ctx = ctx;
		return lt;
	}
	Type *elements;
...
public:
	...
	llvm::StructType *getLLType()
	{
		...
		if (! instanceof <TLCType>(of))
			except(E_UNRESOLVED_TYPE, "Module Instance must refer to module type");
		auto base_mod_ref = ((TLCType *)of)->ref;
		if (!base_mod_ref->refersTo)
			except(E_UNRESOLVED_TYPE, "Module Instance must refer to a resolved module type");
