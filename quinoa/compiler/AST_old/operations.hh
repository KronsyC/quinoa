...
static std::map<PrimitiveType, std::string> primitive_group_mappings { PRIMITIVES_ENUM_GROUPS };
...
enum UnaryOp { UNARY_ENUM_MEMBERS };

enum BinaryOp { INFIX_ENUM_MEMBERS };
...
public:
	...
	default:
	    except(E_BAD_OPERAND, "Cannot get pointer to unary operation of type " + std::to_string(op));
	}
	return nullptr;
    }
...
public:
	...
	case BIN_dot: {
		...
		if(!prop->instance_access)
		    except(E_UNDEFINED_PROPERTY,
		        "Cannot access non-instance property " + prop->str() + " from module instance");
		return prop->type;
	    } else
	...
	if(op == BIN_assignment) {
		...
		if(r->getType()->isPointerTy()) {
			...
			auto size = elType->getArrayNumElements() *
			    (elType->getArrayElementType()->getPrimitiveSizeInBits() / 8);
