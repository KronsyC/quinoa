...
std::unique_ptr<ConstantValue> parse_const(Token tok) {
    ...
    case TT_literal_float:
        return Float::get(std::stold(tok.value));
    default:
        except(E_BAD_EXPRESSION,
               "Failed to generate literal for '" + tok.value +
                   "' with an id of: " + std::to_string(tok.type));
    }
}
...
std::unique_ptr<Expr> parse_expr(std::vector<Token> toks, Scope* parent) {
        ...
        if (target_toks.size()) {
            ...
            if (toks[0].is(TT_l_square_bracket)) {
                ...
                if (toks.size() == 0) {
                    ...
                    auto subs = std::make_unique<Subscript>(
    ...
    if (first.is(TT_identifier)) {
        ...
        auto before = toks;
        if (toks.size() > 1 &&
            (toks[1].is(TT_op_generic) || toks[1].is(TT_double_colon) ||
             toks[1].is(TT_l_paren))) {
    ...
    if (first.is(TT_identifier)) {
        ...
        if (toks.size() &&
            (toks[1].is(TT_double_colon) || toks[1].is(TT_op_generic) ||
             toks[1].is(TT_l_brace))) {
            auto name = parse_long_name(toks);
            ...
            if (toks[0].is(TT_l_brace)) {
                ...
                if (generic_args.size())
                    typ = ParameterizedTypeRef::get(typ, generic_args);
    ...
    for (unsigned int i = 0; i < toks.size(); i++) {
        ...
        if (t.is(TT_op_generic))
            gen_depth++;
        if (gen_depth && t.is(TT_greater))
            decr_gedepth_on_next_iter = true;
    ...
    if (splitPoint == -1 || splitPoint == 0 ||
        splitPoint == (long long)toks.size() - 1) {
        ...
        // Handle postfix operators
            ...
            return Intrinsic<intr_mul>::create(intrinsic_type,
                                               std::move(params), type_args);
        }
    ...
    if (optype == BIN_dot) {
        ...
        if (dynamic_cast<MethodCall*>(rightAST.get())) {
            auto base_call =
                std::unique_ptr<MethodCall>((MethodCall*)rightAST.release());
            if (base_call->name->container)
                except(
                    E_BAD_EXPRESSION,
                    "Instance calls may only be identified by single-part identifiers");
            ...
            auto base_var = std::unique_ptr<SourceVariable>(
                (SourceVariable*)rightAST.release());
            if (base_var->name->parts.len() > 1)
                except(E_BAD_MEMBER_ACCESS,
                       "You may only use simple identifiers to access members");
            auto sub = std::make_unique<MemberAccess>(
                std::move(leftAST),
                std::make_unique<Name>(base_var->name->last()));
            return sub;
        }
    }

    auto binop = std::make_unique<BinaryOperation>(std::move(leftAST),
                                                   std::move(rightAST), optype);
    binop->scope = parent;
    return binop;
}
