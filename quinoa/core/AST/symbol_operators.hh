...
#include "./type.hh"
enum UnaryOpType { UNARY_ENUM_MEMBERS };

enum BinaryOpType { INFIX_ENUM_MEMBERS };
...
class UnaryOperation : public Expr {
  public:
    ...
    std::string str() { return "Some Unary op"; }
...
#define ret(LL_INST, cast_to) return cast({bld.Create##LL_INST(val(cast_to)), type()}, expected);
        ...
        if (!op_t)
            except(E_INTERNAL, "operand has no type: " + operand->str());
        ...
        case PRE_bitwise_and: {
            ...
            if (!ptr_ty)
                except(E_INTERNAL, "assign_ptr returned a non-pointer type");
        ...
        case PRE_minus: {
            ...
            if (op_type.is_signed()) {
                ...
                except(E_BAD_OPERAND,
                       "Cannot invert the sign of an unsigned primitive\n\t\tIn operation: " + this->str());
            }
        }
        ...
        case POST_decrement: {
            ...
            auto updated_val =
                LLVMValue(op_type == POST_increment || op_type == PRE_increment ? bld.CreateAdd(val, one)
                                                                                : bld.CreateSub(val, one),
                          val.type);

            LLVMValue return_val = op_type == POST_increment || op_type == POST_decrement
                                       ? operand->llvm_value(vars, expected)
                                       : updated_val;
        ...
        default:
            except(E_INTERNAL, "Failed to generate llvalue for unary operation: " + std::to_string(op_type));
        }
    }
...
#define C(x, y)                                                                                                        \
  ...
  public:
    ...
    void set_initializing() { this->internal_intrinsic->flags.push_back(FLAG_CONST_INIT_RIGHTS); }

    std::string str() { return this->internal_intrinsic->str(); }

    LLVMValue assign_ptr(VariableTable& vars) { except(E_BAD_ASSIGNMENT, "Binary Operations are not assignable"); }
    ...
    LLVMValue llvm_value(VariableTable& vars, LLVMType expected_type = {}) {
        ...
        if (this->op_type == BIN_assignment) {
            ...
            if (!left_t)
                except(E_INTERNAL, "left operand: " + left->str() + " in operation: " + str() + " has no type");
            if (!right_t)
                except(E_INTERNAL, "right operand: " + left->str() + " in operation: " + str() + " has no type");
    ...
    std::vector<Statement*> flatten() {
        std::vector<Statement*> ret = {this};
        for (auto n : internal_intrinsic->flatten())
            ret.push_back(n);
        return ret;
    }
...
#define right_t return right->llvm_type();
        ...
        case BIN_percent:
        common case BIN_star:
        common case BIN_plus:
        common case BIN_minus:
        common case BIN_bool_or:
        boolean case BIN_bool_and:
        boolean case BIN_dot:
            except(E_INTERNAL, "unreachable");
        case BIN_slash:
        common case BIN_lesser:
        common case BIN_greater:
        common case BIN_lesser_eq:
        common case BIN_greater_eq:
        common case BIN_assignment:
        right_t case BIN_equals:
        common case BIN_not_equals:
        common case BIN_bitwise_and:
        common case BIN_bitwise_or:
        common case BIN_bitwise_xor:
        common case BIN_bitwise_shl:
        common case BIN_bitwise_shr:
            common
        }
#undef common
#undef boolean
...
#define r(x, y)                                                                                                        \
    case BIN_##x:                                                                                                      \
        ...
        switch (this->op_type) {
            r(percent, %) r(star, *) r(plus, +) r(minus, -) r(bool_or, ||) r(bool_and, &&) r(dot, .) r(slash, /)
                r(lesser, <) r(greater, >) r(lesser_eq, <=) r(greater_eq, >=) r(assignment, =) r(equals, ==)
                    r(not_equals, !=) r(bitwise_and, &) r(bitwise_or, |) r(bitwise_xor, ^) r(bitwise_shl, <<)
                        r(bitwise_shr, >>)
#undef r
        }
    }
  ...
  protected:
    _Type get_type() { return this->internal_intrinsic->type(); }
};
...
class MemberAccess : public Expr {
  public:
    ...
    std::vector<Type*> flatten_types() { return {}; }
    std::string str() { return member_of->str() + "." + member_name->str(); }
    ...
    LLVMValue llvm_value(VariableTable& vars, LLVMType expected_type = {}) {
        ...
        if (auto dlt = this->member_of->type()->get<DynListType>()) {
            ...
            if (member_name->str() == "len") {
                ...
                except(E_BAD_MEMBER_ACCESS, "slices only have the 'ptr' and 'len' properties");
        }
  ...
  protected:
    _Type get_type() {
        ...
        if (auto pref = left_t->get<ParameterizedTypeRef>()) {
            ...
            // except(E_INTERNAL, "unsupported ParameterizedTypeRef: " +
            // pref->str());
        }
