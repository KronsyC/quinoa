...
template <typename T, typename U> class Constant : public ConstantValue {
  public:
...
class Boolean : public Constant<bool, Boolean> {
  public:
    ...
    std::string str() { return value ? "true" : "false"; }
  ...
  protected:
    _Type get_type() { return Primitive::get(PR_boolean); }
};
...
class Integer : public Constant<long long, Integer> {
  public:
    ...
    std::string str() { return std::to_string(value); }
...
class Float : public Constant<long double, Float> {
  public:
    ...
    llvm::Constant* const_value(LLVMType expected) {
        ...
        if (!expected->isFloatingPointTy())
            except(E_ERR, "Cannot create a float with a non-float type");
  ...
  protected:
    _Type get_type() { return Primitive::get(PR_float64); }
};
