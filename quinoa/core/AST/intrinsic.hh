...
#include "./primary.hh"
#include "./type.hh"
enum IntrinsicType { INTRINSICS_ENUM_MEMBERS };
...
template <IntrinsicType IT> class Intrinsic : public _Intrinsic {
  public:
...
#define C(intr_ty)                                                                                                     \
    case intr_ty:                                                                                                      \
        return std::make_unique<Intrinsic<(intr_ty)>>(std::move(args), type_args);
...
#undef C
    ...
    _Type get_type() {
        ...
        // Return left type for binary operations (most operations enforce left
        // and right type equality) Error for any other operations

        if (this->args.len() == 1)
            return args[0].type();
        else if (this->args.len() == 2)
            return args[0].type();

        except(E_INTERNAL, "no default implementation for get_type, for intrinsic " + name);
    }
    LLVMValue assign_ptr(VariableTable& vars) { except(E_INTERNAL, "assign_ptr not implemented for intrinsics"); }
        ...
        for (auto entry : defs) {
            if (!entry->alias.empty() && entry->alias[0] == name) {
                ...
                if (args.len() != arg_count) {
                    except(E_BAD_INTRINSIC_CALL, "Bad argument count for intrinsic function: '" + name +
                                                     "'"
                                                     "\n\t\texpected: " +
                                                     std::to_string(arg_count) +
                                                     " args, but got: " + std::to_string(args.len()));
                }
                if (type_args.size() != type_arg_count) {
                    except(E_BAD_INTRINSIC_CALL, "Bad type generic count for intrinsic function: '" + name +
                                                     "'"
                                                     "\n\t\texpected: " +
                                                     std::to_string(type_arg_count) +
                                                     " generic args, but got: " + std::to_string(type_args.size()));
                }
                return;
            }
        }
