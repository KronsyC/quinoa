...
class Name : public Reference {
  ...
  public:
    Name(std::string value) { this->value = value; }
    ...
    void set_name(std::string val) { this->value = val; }
};
...
class LongName : public Reference {
  public:
    ...
    Name& last() { return parts[parts.len() - 1]; }
...
class SourceVariable : public Name, public Expr {
  public:
    ...
    SourceVariable(LongName name) { this->name = std::make_unique<LongName>(name); }

    SourceVariable(std::unique_ptr<LongName> name) { this->name = std::move(name); }
    ...
    std::string str() { return name->str(); }
    ...
    _Type get_type() {
        if (!scope)
            except(E_INTERNAL, "variable " + str() + " has no scope");
