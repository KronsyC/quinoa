...
llvm::Function* make_fn(Method& f, llvm::Module* mod, llvm::Function::LinkageTypes linkage, bool as_resolved_generic) {
    if (f.is_generic() && !as_resolved_generic)
        return nullptr;
    ...
    if (auto re = mod->getFunction(name))
        return re;
...
std::variant<LLVMValue, std::string> try_cast(LLVMValue _val, const LLVMType& _to, bool is_explicit) {
    ...
    if (isInt(val_ty) && isInt(to)) {
        ...
        // - Only allow upcasting, ie new integer must have more bytes than
        // other
        ...
        if (val_ty.is_signed() && !to.is_signed()) {
            // signed -> unsigned, disallowed unless explicit due to possible
            // loss of information
            if (is_explicit) {
                return LLVMValue{builder()->CreateIntCast(val, to, to.is_signed()), to};
            }
            return "Cannot implicitly cast a signed integer to an unsigned "
                   "integer, try casting using the `as` operator";
            ...
            if (to->getPrimitiveSizeInBits() < val_ty->getPrimitiveSizeInBits() && !is_explicit) {
                return "Cannot implicitly downcast an integer, please use the "
                       "explicit casting `as` operator\n\t\t in cast " +
                       val_ty.qn_type->str() + " -> " + to.qn_type->str();
            }
            return LLVMValue{builder()->CreateIntCast(val, to, to.is_signed()), to};
            ...
            return LLVMValue{builder()->CreateIntCast(val, to, false), to};
                   ...
                   "(bug) unhandled integer cast case from " + val_ty.qn_type->str() + " to " + to.qn_type->str());
        }
    }
    ...
    if (val_ref && to.qn_type->get<DynListType>()) {
        ...
        if (auto list_ty = val_ref->of->get<ListType>()) {
            ...
            if (my_element_type != target_element_type)
                return "Cannot cast a list to a slice with a differing element "
                       "type";

            auto list_len = Integer::get(list_ty->llvm_type()->getArrayNumElements())
                                ->const_value(LLVMType(Primitive::get(PR_uint64)));

            auto slice_alloc = create_allocation(to, builder()->GetInsertBlock()->getParent());
    ...
    if (val_ref) {
        // if the target type is a reference, ensure their contents are
        // coercible
        if (to.qn_type->get<ReferenceType>()) {
            except(E_INTERNAL, "Reference -> Reference casting is currently unsupported");
        }
        // otherwise, attempt to cast the referenced data (implicit
        // de-referencing)
    ...
    if (isInt(to) && val_ty.qn_type->get<Ptr>()) {
        if (!is_explicit)
            return "Casting from a pointer to an integer is an explicit "
                   "operation";
        else {
    ...
    if (val_ty.qn_type->get<Ptr>() && to.qn_type->get<Ptr>()) {
        if (!is_explicit)
            return "Casting from a pointer to another pointer is an explicit "
                   "operation (" +
                   val_ty.qn_type->str() + " -> " + to.qn_type->str() + " )";
    ...
    if (isInt(val_ty) && to->isFloatingPointTy()) {
        if (val_ty.is_signed()) {
            ...
            return LLVMValue{builder()->CreateUIToFP(val, to), to};
    }

    // float -> int, explicit only
    if (isInt(to) && val_ty->isFloatingPointTy()) {
        if (!is_explicit)
            return "Casting from a float to an integer is an explicit "
                   "operation";
        if (to.is_signed())
            return LLVMValue{builder()->CreateFPToSI(val, to), to};
        else
            return LLVMValue{builder()->CreateFPToUI(val, to), to};
    }

    // float -> float, implicit grow, explicit shrink
    if (to->isFloatingPointTy() && val_ty->isFloatingPointTy()) {
        if (to->getPrimitiveSizeInBits() < val_ty->getPrimitiveSizeInBits() && !is_explicit) {
            return "Cannot implicitly downcast a float, please use the "
                   "explicit casting `as` operator\n\t\t in cast " +
                   val_ty.qn_type->str() + " -> " + to.qn_type->str();
        }
